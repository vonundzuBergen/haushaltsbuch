"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var info_box_service_1 = require("./info-box.service");
var InfoBoxComponent = (function () {
    function InfoBoxComponent(_infoBoxService) {
        var _this = this;
        this._infoBoxService = _infoBoxService;
        this._infoBoxService.showError$.subscribe(function (x) {
            _this.showInfoBox(x);
        });
    }
    InfoBoxComponent.prototype.showInfoBox = function (show) {
        console.log("in showInfoBox");
        $("#infoModal").modal("show");
    };
    return InfoBoxComponent;
}());
InfoBoxComponent = __decorate([
    core_1.Component({
        selector: 'info-box',
        templateUrl: '/app/info-box.component.html',
        styleUrls: ['app/info-box.component.css']
    }),
    __metadata("design:paramtypes", [info_box_service_1.InfoBoxService])
], InfoBoxComponent);
exports.InfoBoxComponent = InfoBoxComponent;
//# sourceMappingURL=info-box.component.js.map